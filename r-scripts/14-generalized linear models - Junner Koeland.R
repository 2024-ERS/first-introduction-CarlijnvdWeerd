...
plot(m1,which=2) # if approx a straight line, then it is good, if not normal, maybe transform the model or use a different model
...
# gaussian is the same as normal distribution, identity is the values of the data
...
# here is random slopes model 
m8 <- lmerTest::lmer(specrich~graztreat + (graztreat|block), data=dat)
coef(m8)
summary (m8)
anova(m8)
AIC(m8,m7) # which model is better?
# Significant difference between the AIC, the treatments depends on the block
...
# if the design allows it you can try to fit it but in this case it does not fit anymore
anova(m11,m10)
