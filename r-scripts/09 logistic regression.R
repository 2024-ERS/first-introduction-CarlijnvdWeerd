...
dat2
p1 <- ggplot2::ggplot(data=dat2, aes(x=Point_ID, y=layer_thickness_cm, 
      fill=soil_layer)) +
       geom_area() + 
       coord_cartesian(ylim=c(95,125)) # how to zoom in on certain part of the graph without removing any data
p1

# calculate the frequency (as a proportion) of occurrence of each species 
# in 2023, and sort according to frequency 
### Nice for using in manuscript!!!!!
dat3<-dat1 |>
  dplyr::filter(presence==1) |>
  mutate(speciesnum=-as.numeric(factor(species)))
p2 <- ggplot(data=dat3, aes(x=Point_ID, y=speciesnum, 
      col=species)) +
       geom_point(size=2)
p2
...
dat1<-dat1 |>
  dplyr::mutate(predicted=ifelse(species==specsel,
                                 predict(m3,type="response"),NA))
p4+ geom_line(data=dat1 |> dplyr::filter(!is.na(predicted)),
               aes(y=predicted,col=species),linewidth=1.3)
