...
vdatl<- vdat |>
  ...
  dplyr::filter(!Species_ID %in% c("bare","litter", "mosses", 
                            "SalicEur", "SalicPro"))  # ! in front means opposite
...
#merge the files 
vdat3<-left_join(vdatl,vdatt, by="Species_ID")
...
vdat3 |>
  ...
  ggplot2::ggplot(aes(x=factor(TransectPoint_ID),y=factor(-year),fill = cover)) + geom_tile() + 
